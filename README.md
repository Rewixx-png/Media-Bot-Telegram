...
4.  Скрипт автоматически:
    - Установит все необходимые системные зависимости (C++, CMake, Python, FFmpeg, Node.js, PM2 и т.д.).
    - Соберет из исходного кода локальный сервер Telegram Bot API.
    - Создаст и запустит `systemd`-сервис для API-сервера, чтобы он работал постоянно.
...
**API-сервер** (управляется через `systemd`):
```bash
# Проверить статус
